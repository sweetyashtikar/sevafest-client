...
export const ProductApi = {
  ...
  getVendorProducts: (params = {}) =>
    apiClient("/product/vendor/my-products", { method: "GET", params }),
};

export const zipcodeService = {
  // GET /zipCode?page=1&limit=10&city_id=...&is_deliverable=...
  getZipcodes: (page = 1, limit = 10, filters = {}) => {
    const params = new URLSearchParams({ page, limit });
 
    // Only append filters if they have a value
    if (filters.city_id) params.append("city_id", filters.city_id);
    if (filters.is_deliverable !== "" && filters.is_deliverable !== undefined)
      params.append("is_deliverable", filters.is_deliverable);
 
    return apiClient(`/zipCode?${params}`, {
      method: "GET",
    });
  },
  ...
  // POST /zipCode/bulk  — body: { zipcodes: [...] }
  bulkUploadZipcodes: (zipcodes) =>
    apiClient("/zipCode/bulk", {
      method: "POST",
      body: { zipcodes },
    }),
 
  // DELETE /zipCode/bulk/delete  — body: { zipcode: [id1, id2, ...] }
  // NOTE: backend expects key "zipcode" (not "ids") for bulk delete
  bulkDeleteZipcodes: (ids) =>
    apiClient("/zipCode/bulk/delete", {
      method: "DELETE",
      body: { zipcode: ids },
    }),
 
  // GET /zipCode/check/:zipcode
  checkAvailability: (zipcode) =>
    apiClient(`/zipCode/check/${zipcode}`, {
      method: "GET",
    }),
 
  // GET /zipCode/check/delivery-true
  checkAllDeliverable: () =>
    apiClient("/zipCode/check/delivery-true", {
      method: "GET",
    }),
 
  // GET /zipCode/:cityId  (uses the /:id route — backend populates by city_id query internally via getZipcodesByCity)
  getZipcodesByCity: (cityId) =>
    apiClient(`/zipCode/${cityId}`, {
      method: "GET",
    }),
};
